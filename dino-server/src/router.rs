...
impl AppRouter {
    pub fn match_it<'m, 'p>(
        ...
        method: Method,
        path: &'p str,
    ) -> Result<Match<'m, 'p, &'p str>, AppError>
    where
        'm: 'p,
    {
...
mod tests {
    ...
    #[test]
    fn match_it_param_should_work() {
        let mut router = Router::new();
        router.insert("/api/:name", "hello").unwrap();
        let m = router.at("/api/world").unwrap();
        assert_eq!(*m.value, "hello");
        assert_eq!(m.params.get("name"), Some("world"));
    }
}
